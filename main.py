...
async def on_fetch(request, env, ctx):
    try:
        ...
        if path == "" or path == "/" or path == "/index":
            try:
                ...
                <body>
                    ...
                    <hr>
                    <ul>
                        {html_list if html_list else "<li>Belum ada artikel berita yang digenerate. Silakan tunggu jadwal cron bot berjalan atau masukkan data contoh ke database.</li>"}
                    </ul>
                </body>
                </html>
                """
                ...
                # Jika tabel kosong atau ada kendala kueri, tampilkan pesan ramah alih-alih Error 1101
                return Response.new(f"<h1>Website Aktif</h1><p>Belum ada data artikel atau tabel belum terisi: {str(db_err)}</p>", headers={"Content-Type": "text/html;charset=UTF-8"}, status=200)

        # Halaman Detail Artikel
        elif path.startswith("/article/"):
            slug = path.split("/")[-1]
            query = "SELECT title, content, created_at FROM articles WHERE slug = ?"
            stmt = env.DB.prepare(query).bind(slug)
            article = await stmt.first()
            
            if not article:
                return Response.new("Artikel tidak ditemukan", status=404)
        ...
        return Response.new(f"Worker Error: {str(e)}", status=500)
