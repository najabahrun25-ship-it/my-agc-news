...
# Fungsi pembantu untuk membuat URL slug yang ramah SEO
def slugify(text):
    text = text.lower()
...
# 1. Menangani Pengunjung yang Membuka Website
async def on_fetch(request, env, ctx):
    url_path = str(request.url).split("/")[-1]
    
    # Halaman Utama (Menampilkan Daftar Berita)
    if url_path == "" or url_path == "index":
        try:
            # Ambil 10 berita terbaru dari database D1
    ...
    elif "article/" in str(request.url):
        slug = url_path
...
# 2. Menangani Proses Otomatis Bot (Berjalan Berdasarkan Jadwal Cron)
async def on_scheduled(event, env, ctx):
    # Tempat logika bot: Ambil RSS -> Rewrite pakai AI -> Simpan ke database D1
